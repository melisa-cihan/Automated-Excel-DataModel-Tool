...
/**
 * org.melisa.datamodel.normalization.SecondNormalizer is responsible for
 * transforming a dataset from First Normal Form (1NF)
 * into Second Normal Form (2NF). This process involves identifying candidate
 * keys,
 * analyzing functional dependencies, and decomposing the original relation to
 * eliminate
...
public class SecondNormalizer {
    ...
    /**
     * Helper to sanitize column names using the logic from
     * org.melisa.datamodel.io.SqlGenerator.
     * We call this to ensure key names are stored in the same format
     * org.melisa.datamodel.io.SqlGenerator will use.
     *
     ...
     *
     * @param input1NFData  The list of maps representing the data that is already
     *                      in 1NF.
     * @param tableNameBase The user-provided base name (e.g., "shop") used for
     *                      prefixing.
     * @return A list of org.melisa.datamodel.model.DecomposedRelation objects, each
     *         containing the data and key metadata (PK/FK).
     */
    public List<DecomposedRelation> normalizeTo2NF(
        ...
        if (candidateKey.isEmpty()) {
            System.err.println(
                    "Error: No Candidate Key could be identified for the relation. Returning original data as MainRelation.");
     ...
     *
     * @param allCandidateKeys The set of all identified minimal candidate keys.
     * @return The selected candidate key as a set of attribute names, or an empty
     *         set if no keys exist.
     */
     ...
     * and correctly identifies the Primary Key (PK) and Foreign Key (FK) for each
     * new relation.
     *
     * @param input1NFData     The input relation (List of Maps).
     * @param candidateKey     The identified composite primary key (e.g., [MiNr,
     *                         ProNr]).
     * @param sqlTableNameBase The SQL-sanitized base name (e.g., "SHOP") to build
     *                         FK references.
    ...
    private List<DecomposedRelation> decomposeForPartialDependencies(
        ...
        if (sqlCandidateKey.size() <= 1) {
            ...
            String relationName = sqlTableNameBase + "_" + toSqlIdentifier(MAIN_RELATION_NAME);
            normalizedRelations
                    .add(new DecomposedRelation(relationName, input1NFData, sqlCandidateKey, Collections.emptyMap()));
        ...
        System.out.println("Decomposed Relation: " + detailsRelationInternalName + " created with attributes: "
                + dependentAttributes);
     ...
     * Checks if attribute 'dependentAttr' is functionally dependent on
     * 'determinant'.
     *
     * @param data          The input relation data.
     * @param determinant   The single attribute that is a part of the composite
     *                      primary key.
     * @param dependentAttr The non-prime attribute to check for partial dependency.
     * @return true if the dependent attribute is functionally determined by the
     *         partial determinant.
     */
